...
function daysUntil(value?: string | null) {
  ...
  return Math.ceil((ts - startOfToday()) / (1000 * 60 * 60 * 24));
}

function vaccineNotifications(vaccinations: any[], link: string): NotificationItem[] {
  return vaccinations
    .filter((v) => v.next_due && daysUntil(v.next_due) <= 30)
    .map((v) => {
...
function appointmentNotifications(
  ...
  return appointments
    .filter((a) => isScheduledStatus(a.status) && (parseDate(a.date) ?? Infinity) >= today)
    .map((a) => {
  ...
  return appointments
    ...
    .map((a) => {
      ...
      return {
        ...
        type: "alert" as const,
        time: formatDate(a.date),
        sortKey: -500 + (parseDate(a.date) ?? 0),
        link,
      };
    });
}
...
function inventoryNotifications(items: any[]): NotificationItem[] {
  ...
  for (const item of items) {
    ...
    const expTs = parseDate(item.expiration_date);

    if (expTs !== null && expTs < today) {
      alerts.push({
        id: `inv-exp-${item.id}`,
...
function dewormingNotifications(dewormings: any[]): NotificationItem[] {
  return dewormings
    .filter((d) => {
      ...
      return d.next_due && daysUntil(d.next_due) <= 14;
    })
    .map((d) => {
      const days = daysUntil(d.next_due);
      const overdue = days < 0;
